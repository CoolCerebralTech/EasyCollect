...
export const CreateRoomPage: React.FC = () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        LocalStorageService.saveRoom({
          ...
          title: data.title,
          organizerToken: roomData.organizer_token,
          role: 'organizer',
        ...
        showToast({
          type: 'error',
          message: result.error.message || 'Failed to create room. Check your connection and try again.',
        });
      }
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'An unexpected error occurred. Check your connection and try again.';
      showToast({
        type: 'error',
        message: msg,
      });
  ...
  const handleContinue = () => {
    if (createdRoom && hasSavedAdminKey) {
      navigate(`/room/${createdRoom.organizer_token}`);
    }
  };
  ...
  const handleWhatsAppShare = () => {
    ...
    const viewerUrl = TokenUtils.generateRoomUrl(createdRoom.contributor_token);
...
_Note: Payments are still sent to the treasurer via M-Pesa as usual._`;
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-6 sm:py-12 px-4 font-sans">
      <div className="max-w-2xl mx-auto">
        <div className="mb-6 sm:mb-8">
          <button
            onClick={() => navigate('/')}
            className="inline-flex items-center text-slate-500 hover:text-slate-800 transition-colors mb-4 sm:mb-6 text-sm font-medium min-h-[44px] -ml-1 px-1"
          >
        ...
        <div className="bg-white rounded-2xl shadow-xl shadow-slate-200 border border-slate-100 p-5 sm:p-10">
        ...
        >
          <div className="space-y-6 sm:space-y-8 px-1 sm:px-2">
            <div className="text-center pb-2">
              <div className="w-16 h-16 sm:w-20 sm:h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4 animate-bounce">
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 sm:p-6 relative overflow-hidden">
              ...
              <div className="flex items-start gap-3 sm:gap-4 mb-4 mt-1">
                <div className="p-2 sm:p-3 bg-amber-100 rounded-lg text-xl sm:text-2xl shrink-0">👑</div>
                <div>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <div className="flex-1 relative">
                  <input
                    type="text"
                    value={TokenUtils.generateRoomUrl(createdRoom.organizer_token)}
                    readOnly
                ...
                {/* Variant prop is now accepted by the component below */}
                <CopyButton
                  text={TokenUtils.generateRoomUrl(createdRoom.organizer_token)}
              ...
              <div className="mt-5 flex items-start gap-3 p-3 bg-white/50 rounded-lg border border-amber-100 cursor-pointer min-h-[48px]" onClick={() => setHasSavedAdminKey(!hasSavedAdminKey)}>
                <div className={`mt-0.5 w-5 h-5 rounded border flex items-center justify-center transition-colors shrink-0 ${hasSavedAdminKey ? 'bg-amber-500 border-amber-500 text-white' : 'bg-white border-slate-300'}`}>
                ...
                <label className="text-sm text-slate-700 select-none cursor-pointer font-medium leading-relaxed">
                  I have saved this Admin Link securely.
                </label>
              </div>
            </div>

            <div className="bg-green-50 border border-green-200 rounded-xl p-5 sm:p-6 relative overflow-hidden">
              ...
              <div className="flex items-start gap-3 sm:gap-4 mb-4 mt-1">
                <div className="p-2 sm:p-3 bg-green-100 rounded-lg text-xl sm:text-2xl shrink-0">📢</div>
                <div>
              ...
              <div className="flex flex-col gap-3">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 px-5 sm:px-6 py-4 bg-[#25D366] hover:bg-[#128C7E] text-white font-bold rounded-xl transition-all shadow-md hover:shadow-lg transform active:scale-95 min-h-[52px]"
                >
                ...
                <div className="flex gap-2">
                  <input
                    type="text"
                    value={TokenUtils.generateRoomUrl(createdRoom.contributor_token)}
                    readOnly
                    className="flex-1 px-4 py-2 border border-green-200 rounded-lg text-xs bg-white text-gray-500"
                    onClick={(e) => e.currentTarget.select()}
                  />
                  <CopyButton
                    text={TokenUtils.generateRoomUrl(createdRoom.contributor_token)}
                    label="Copy"
            ...
            <div className="pt-2 sm:pt-4 flex flex-col items-center gap-4 pb-2">
              <Button
                onClick={handleContinue}
                fullWidth
                size="lg"
                disabled={!hasSavedAdminKey}
                className={`font-bold py-4 text-base sm:text-lg transition-all min-h-[56px] ${
                  hasSavedAdminKey 
